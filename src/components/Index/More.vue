...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 下来刷新加载
      loadFrist() {
        this.$axios.get("/current/page?pageSize=10&pageNo=" + this.courrentPage + '&cid='+this.$route.query.id )
          .then(res => {
            console.log(res.data.data)
            this.courrentPage == 1;
            this.allLoaded = false; // 可以进行上拉
            this.HalfPricePerDay = res.data.data;
      ...
      loadMore() {
        ...
        this.$axios.get("/current/page?pageSize=10&pageNo=" + this.courrentPage + '&cid='+this.$route.query.id )
          .then(res => {
            // concat数组的追加
            this.HalfPricePerDay = this.HalfPricePerDay.concat(res.data.data);
